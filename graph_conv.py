...
from nn import Layer, LSTM, apply_dropout, CRFForward, GraphCNN
...
class GraphConvCRF:
	...
	#dropout2 = theano.shared(
        #        np.float64(0.2).astype(theano.config.floatX)
        #    )
	...
	#if args.word_bidirect:
	#    layers.append(Layer(
	#    	n_in = word_lstm_dim,
	#    	n_out = word_lstm_dim,
	#    	activation = tanh
	#	))
	#    prev_output = layers[-1].forward(prev_output)
	...
	if args.conv != 0:
	    for ind in range(args.clayer):
	    	...
	    	prev_output = layers[-1].forward_all(prev_outputs[-1], up_ids, up_id_masks, bv_ur_slicess[ind], b_ur_slicess[ind], down_ids, down_id_masks, bv_dr_slicess[ind], b_dr_slicess[ind])
		#prev_outputs.append(apply_dropout(prev_output, dropout, v2=True))
		prev_outputs.append(prev_output)

	#prev_output = T.concatenate(prev_outputs[1:], axis =1)
	prev_size = args.clayer * prev_size
	...
	for i in range(nC):
	    ...
	    prev_outputs.append(layers[-1].forward(prev_output))

	prev_output_ = T.stack(prev_outputs)
	layers.append(Layer(
	    n_in = prev_size,
	    n_out = nC,
	    activation = softmax
	))
	weights = layers[-1].forward(prev_output)
	tags_scores = T.mean(weights.dimshuffle(1, 0, 'x') * prev_output_, axis = 0)
	...
	for l, i in zip(layers[2+len(r_emb_layers):], range(len(layers[2+len(r_emb_layers):]))):
