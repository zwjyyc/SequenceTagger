...
from nn import Layer, LSTM, apply_dropout, CRFForward, GraphCNN, GraphCNNTensor
from utils import say, shared
from optimization import Optimization

class GraphConvCRFV4:
    def __init__(self, args, w_emb_layer, c_emb_layer, r_emb_layers, r_matrix_layers):
	self.args = args
	self.w_emb_layer = w_emb_layer
	self.c_emb_layer = c_emb_layer
	self.r_emb_layers = r_emb_layers
	self.r_matrix_layers = r_matrix_layers	
	...
	if args.conv != 0:
		...
		residual = True
                if ind == 0:
		    residual = False
		prev_output = layers[-1].forward_all(prev_output, up_ids, up_id_masks, bv_ur_slicess[ind], bv_ur_matrixss[ind], b_ur_slicess[ind], b_ur_matrixss[ind], down_ids, down_id_masks, bv_dr_slicess[ind], bv_dr_matrixss[ind], b_dr_slicess[ind], b_dr_matrixss[ind], residual = residual)
		prev_output = apply_dropout(prev_output, dropout)
